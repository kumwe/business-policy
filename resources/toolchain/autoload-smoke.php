...
$loader = require $argv[1] ?? dirname(__DIR__, 2) . '/vendor/autoload.php';
...
foreach (array_keys($manifest['symbols']) as $symbol) {
    if (!class_exists($symbol) && !interface_exists($symbol) && !enum_exists($symbol)) {
        throw new RuntimeException('Cannot autoload ' . $symbol);
    }
}
echo 'Autoload smoke passed for ' . count($manifest['symbols']) . " public symbols.\n";
