...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-business-policy-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-business-policy',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-business-policy.zip';
if (!is_file($archive)) {
...
$installed = $consumer . '/vendor/kumwe/business-policy';
...
consumerRun([PHP_BINARY, $installed . '/examples/policy.php', $autoload], $workspace);
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\BusinessPolicy\\Tests\\')
    ) {
